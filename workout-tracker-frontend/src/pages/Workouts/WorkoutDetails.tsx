...
const WorkoutDetails = () => {
  ...
  const renderActionButtons = () => {
    if (workout.isCompleted) {
      return null; // No buttons if workout is already completed
    }
  
    if (user?.role === 'Coach') {
      // Coach buttons - can only add exercises
      return (
      ...
      // User buttons - can only complete workout
      return (
        <button 
          onClick={completeWorkout}
          disabled={completingWorkout}
          className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-lg text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {completingWorkout ? (
            <>
  ...
  return (
    <div className="min-h-screen bg-slate-900">
      ...
      <main className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        {showAddExercise && user?.role === 'Coach' && (
